...
def confirm_prediction(row):
    ...
    # CMF
    if row['CMF'] >= -0.004935601531600464:
        strong_signals += 1

    # ADX 36.082765757882896
    if row['ADX'] >= 22.523426795906914:
        strong_signals += 1

    # RSI
    if 39.64334705075517 <= row['RSI_lag_10'] <= 60.59907834101579:
        strong_signals += 1

    # Volatility 0.0015255693135834839
    if row['Volatility_20'] >= 0.0008333715753742228:
        strong_signals += 1

    # Volume_Ratio 1.7542492077748215
    if row['Volume_Ratio'] >= 0.8832762146127926:
        strong_signals += 1

    # ATR
    if row['ATR'] >= 0.002215251727417138:
        strong_signals += 1

    # Liquidity_Index 0.7417887272414238
    if row['Liquidity_Index'] >=  0.5278289060516576:
        strong_signals += 1

    # Confirm if majority of signals are strong
    return 'confirm' if strong_signals >= 4 else 'not_strong'
